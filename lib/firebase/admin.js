...
function parseBase64PrivateKey(value) {
  const cleaned = normalizeEnvValue(value).replace(/\s+/g, "");
  if (!cleaned) {
    return "";
  }
...
function getServiceAccountFromEnv() {
  ...
  if (json) {
    try {
      ...
      if (parsed.project_id && parsed.client_email && looksLikePemPrivateKey(parsed.private_key)) {
        return parsed;
      }
    } catch {
      // Continue to other env formats if JSON is invalid.
    }
  }
  ...
  const decodedBase64Key = parseBase64PrivateKey(privateKeyBase64);
  const envPrivateKey = normalizePrivateKey(process.env.FIREBASE_PRIVATE_KEY);

  let privateKey = "";
  if (looksLikePemPrivateKey(decodedBase64Key)) {
    privateKey = decodedBase64Key;
  } else if (looksLikePemPrivateKey(envPrivateKey)) {
