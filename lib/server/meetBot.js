...
export async function joinMeetAndCaptureTranscript({
  ...
  try {
    ...
    } else if (/error while loading shared libraries|libglib-2\.0\.so\.0/i.test(message)) {
      message =
        "Playwright Chromium found missing Linux runtime libraries (for example libglib2.0-0). " +
        "Install Playwright OS dependencies in the Railway image (nixpacks aptPkgs) and redeploy.";
    }
